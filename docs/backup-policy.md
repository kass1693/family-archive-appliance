...
백업 대상은 originals와 metadata(DB)입니다.

thumbnails와 medium은 원본으로부터 재생성 가능한 캐시입니다. 백업하지 않습니다.

metadata는 originals와 함께 백업합니다. HDD 장애 시 originals만 복구하면 원본 파일명, 업로드 시각, 삭제 상태 등 메타데이터가 손실됩니다.
...
metadata는 실행 중인 MongoDB를 직접 rsync하지 않습니다. `mongodump`로 일관된 archive를 먼저 생성한 뒤 rsync합니다.
...
2. metadata snapshot 생성
   mongodump --uri="mongodb://mongodb:27017/family-archive" \
     --archive=/tmp/family-archive-YYYY-MM-DD.archive \
     --gzip

3. metadata snapshot rsync
   rsync -av /tmp/family-archive-YYYY-MM-DD.archive \
...
```
<backup-mount>/metadata/
├── family-archive-2025-01-01.archive
├── family-archive-2025-01-02.archive
└── ...
```
