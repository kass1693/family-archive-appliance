...
│   └── {UUID}_medium.{ext}
├── metadata/         보호 수준: 높음   백업: 필수     소유자: API
...
backup-runner는 `/srv/family-archive/originals`와 운영 SQLite DB를 변경하지 않습니다. SQLite snapshot 생성 및 백업 HDD로의 쓰기는 수행합니다.
...
- metadata(SQLite)를 `sqlite3 .backup`으로 snapshot 생성 후 백업 HDD에 rsync
...
2. sqlite3 .backup으로 metadata snapshot 생성 (`/tmp/family-archive-YYYY-MM-DD.sqlite`)
...
sequenceDiagram
    ...
    alt backup 실행 중
        ...
        loop 경과된 날짜 디렉토리 각각
            ...
            api->>api: 해당 날짜 SQLite 레코드 일괄 삭제
        end
...
sequenceDiagram
    ...
    alt 마운트 안 됨
        ...
        backup->>fs: sqlite3 .backup → metadata snapshot 생성
        backup->>hdd: rsync metadata snapshot (family-archive-YYYY-MM-DD.sqlite)
